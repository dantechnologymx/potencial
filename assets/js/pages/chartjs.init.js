...
!function($) {
    ...
    ChartJs.prototype.init = function() {
        ...
        let arr1 = data.split(',');
        console.log(arr1);
        var datalabel = $('#chart-data-label').val();
        datalabel = datalabel.substring(0, datalabel.length-1);
        let arr2 = datalabel.split(',');
        ...
        var lineOpts = {
            scales: {
                yAxes: [{
                    ticks: {
                        max: max(arr1) + max(arr1)*.1,
                        min: min(arr1) - max(arr1)*.1,
                        stepSize: ((max(arr1) + max(arr1)*.1) - min(arr1) ) / 10
                    }
                }]
            }
        };
